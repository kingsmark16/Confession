...
- Read the forwarded visitor IP only in memory for the `ipapi.co` lookup. Persist only nullable `country` and `city` values. Never log or store the raw IP, exact coordinates, or provider payload.
